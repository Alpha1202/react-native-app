...
const StartGameScreen = (props) => {
	...
	const confirmInputHandler = () => {
		if (enteredValue === '' || enteredValue === undefined || enteredValue === null) {
			return Alert.alert('No number selected', 'please select a number between 1 and 99', [
				{ text: 'Okay', style: 'destructive', onPress: resetInputHandler }
			]);
		}
		const chosenNumber = parseInt(enteredValue);
		if (isNaN(chosenNumber) || chosenNumber <= 0 || chosenNumber > 99) {
			return Alert.alert('Invalid number selected', 'please select a number between 1 and 99', [
				{ text: 'Okay', style: 'destructive', onPress: resetInputHandler }
			]);
		}
		setConfirmed(true);
